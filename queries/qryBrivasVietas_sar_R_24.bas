...
Begin OutputColumns
    ...
    Alias ="ne-LV"
    Expression ="Sum(IIf([Valsts]=\"Latvija\",Null,IIf(IsNull([Rezervisti_24].[CKods]),Null,1)))"
    Alias ="BATStud"
    ...
    Alias ="cits"
    Expression ="Sum(IIf(IsNull([dbo_Viesn_Rezervesana].[Tips]),IIf(IsNull([Rezervisti_24].[CKods"
        "]),Null,1),Null))"
End
Begin Joins
    LeftTable ="Rezervisti_24"
    RightTable ="dbo_Personas"
    Expression ="Rezervisti_24.CKods = dbo_Personas.Personas_ID"
    Flag =2
    LeftTable ="dbo_viesn_kl_Istabas"
    RightTable ="Rezervisti_24"
    Expression ="dbo_viesn_kl_Istabas.Istaba = Rezervisti_24.Istaba"
    Flag =2
    LeftTable ="Rezervisti_24"
    RightTable ="VIA_StudStatData"
    Expression ="Rezervisti_24.CKods = VIA_StudStatData.CKods"
    Flag =2
End
Begin OrderBy
    Expression ="dbo_viesn_kl_Istabas.IstabasNr"
...
dbMemo "Filter" ="(([qryBrivasVietas_sar_R_24].[Brīvās vietas]<>0 Or [qryBrivasVietas_sar_R_24].[B"
    "rīvās vietas] IS Null)) AND ([qryBrivasVietas_sar_R_24].[Brīvās vietas]<>4 Or [q"
    "ryBrivasVietas_sar_R_24].[Brīvās vietas] IS Null)"
Begin
    Begin
        dbText "Name" ="S"
