...
Begin OutputColumns
    ...
    Expression ="[dbo_viesn_kl_Istabas].[vietusk]-(select Count([dbo_Viesn_Rezervesana].[Istaba])"
        " from dbo_Viesn_Rezervesana where dbo_Viesn_Rezervesana.Rez_MG=2022 and dbo_Vies"
    ...
    Alias ="ne-LV"
    Expression ="Sum(IIf([Valsts]=\"Latvija\",Null,IIf(IsNull([Rezervisti_22].[CKods]),Null,1)))"
    Alias ="BATStud"
    ...
    Alias ="cits"
    Expression ="Sum(IIf(IsNull([dbo_Viesn_Rezervesana].[Tips]),IIf(IsNull([Rezervisti_22].[CKods"
        "]),Null,1),Null))"
End
Begin Joins
    LeftTable ="Rezervisti_22"
    RightTable ="dbo_Personas"
    Expression ="Rezervisti_22.CKods = dbo_Personas.Personas_ID"
    Flag =2
    LeftTable ="dbo_viesn_kl_Istabas"
    RightTable ="Rezervisti_22"
    Expression ="dbo_viesn_kl_Istabas.Istaba = Rezervisti_22.Istaba"
    Flag =2
    LeftTable ="Rezervisti_22"
    RightTable ="VIA_StudStatData"
    Expression ="Rezervisti_22.CKods = VIA_StudStatData.CKods"
    Flag =2
End
Begin OrderBy
    Expression ="dbo_viesn_kl_Istabas.IstabasNr"
