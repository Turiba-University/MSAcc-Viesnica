SELECT
  ...
  [dbo_viesn_kl_Istabas].[vietusk] -(
    ...
    where
      ...
      and dbo_Viesn_Rezervesana.rez_mg = [forms]![jauns_rezervists]![R_MG]
...
FROM
  (
    qryBrivasVietasRez
    RIGHT JOIN dbo_viesn_kl_Istabas ON [qryBrivasVietasRez].[Istaba] = [dbo_viesn_kl_Istabas].[Istaba]
  )
  LEFT JOIN dbo_Personas ON [qryBrivasVietasRez].[CKods] = [dbo_Personas].[Personas_ID]
GROUP BY
  [qryBrivasVietasRez].[Rez_MG],
  [dbo_viesn_kl_Istabas].[Istaba],
  [dbo_viesn_kl_Istabas].[IstabasNr],
