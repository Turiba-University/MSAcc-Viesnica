...
FROM
  (
    (
      ...
      LEFT JOIN Rezervisti_17 ON [dbo_viesn_kl_Istabas].[Istaba] = [Rezervisti_17].[Istaba]
    )
    LEFT JOIN qryBrivasVietasAiznVSk_17 ON [dbo_viesn_kl_Istabas].[Istaba] = [qryBrivasVietasAiznVSk_17].[Istaba]
  )
  LEFT JOIN dbo_Personas ON [Rezervisti_17].[CKods] = [dbo_Personas].[Personas_ID]
GROUP BY
  [dbo_viesn_kl_Istabas].[Istaba],
  [dbo_viesn_kl_Istabas].[IstabasNr],
