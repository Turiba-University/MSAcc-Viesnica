...
Begin OutputColumns
    ...
    Expression ="[dbo_viesn_kl_Istabas].[vietusk]-(select Count([dbo_Viesn_Rezervesana].[Istaba])"
        " from dbo_Viesn_Rezervesana where dbo_Viesn_Rezervesana.Rez_MG=2020 and dbo_Vies"
    ...
    Alias ="ne-LV"
    Expression ="Sum(IIf([Valsts]=\"Latvija\",Null,IIf(IsNull([Rezervisti_20].[CKods]),Null,1)))"
    Alias ="BATStud"
    ...
    Alias ="cits"
    Expression ="Sum(IIf(IsNull([dbo_Viesn_Rezervesana].[Tips]),IIf(IsNull([Rezervisti_20].[CKods"
        "]),Null,1),Null))"
End
Begin Joins
    LeftTable ="Rezervisti_20"
    RightTable ="dbo_Personas"
    Expression ="Rezervisti_20.CKods = dbo_Personas.Personas_ID"
    Flag =2
    ...
    Expression ="Rezervisti_20.CKods = VIA_StudStatData.CKods"
    Flag =2
End
Begin OrderBy
    Expression ="dbo_viesn_kl_Istabas.IstabasNr"
