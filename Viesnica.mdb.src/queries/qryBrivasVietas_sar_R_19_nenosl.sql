SELECT
  ...
  (
    ...
    where
      ...
      AND (
        (dbo_Viesn_Rezervesana.Ligums)= 0
        or (dbo_Viesn_Rezervesana.Ligums) Is Null
      );
  ) AS RezVietas,
  ...
  Sum(
    IIf(
      ...
      Null,
      IIf(
        IsNull([Rezervisti_19_nenosl].[CKods]),
        Null,
        1
      )
    )
...
FROM
  (
    ...
    LEFT JOIN Rezervisti_19_nenosl ON dbo_viesn_kl_Istabas.[Istaba] = Rezervisti_19_nenosl.[Istaba]
  )
  LEFT JOIN VIA_StudStatData ON Rezervisti_19_nenosl.CKods = VIA_StudStatData.CKods
WHERE
  (
    (
      (Rezervisti_19_nenosl.Ligums) Is Null
      Or (Rezervisti_19_nenosl.Ligums)= 0
    )
  )
GROUP BY
  dbo_viesn_kl_Istabas.Istaba,
